...
//				mScroller.startScroll(0, 0, 255, 0, 1000);
    ...
    private void initData(Context context, AttributeSet attrs) {
        ...
        if (context != null && attrs != null) {
            ...
            mMaxBezierHeight = a.getDimension(R.styleable.FancyIndexer_maxBezierHeight,
                    mMaxBezierHeight);
            mMaxBezierWidth = a.getDimension(R.styleable.FancyIndexer_maxBezierWidth,
                    mMaxBezierWidth);
            mMaxBezierLines = a.getInteger(R.styleable.FancyIndexer_maxBezierLines,
                    mMaxBezierLines);
            mAdditionalTipOffset = a.getDimension(R.styleable.FancyIndexer_additionalTipOffset,
...
//	    	   canvas.drawLine(0, yPos, width, yPos, mPaint);
        ...
        if (dis > -mMaxBezierWidth && dis < mMaxBezierWidth) {
            ...
            if (dis > mMaxBezierWidth / 4) {
                for (int i = mMaxBezierLines - 1; i > 0; i--) {
                    ...
                    if (dis > -mBezier1[i].y && dis < -mBezier1[i - 1].y) {
                        return (dis + mBezier1[i].y) * (mBezier1[i - 1].x - mBezier1[i].x) /
                                (-mBezier1[i - 1].y + mBezier1[i].y) + mBezier1[i].x;
                    }
                }
                return mBezier1[0].x;
            }
            ...
            for (int i = 0; i < mMaxBezierLines - 1; i++) {
                ...
                if (dis > mBezier2[i].y && dis < mBezier2[i + 1].y) {
                    return (dis - mBezier2[i].y) * (mBezier2[i + 1].x - mBezier2[i].x) /
                            (mBezier2[i + 1].y - mBezier2[i].y) + mBezier2[i].x;
                }
            }
    ...
    private void calculateBezierPoints() {
        ...
        for (int i = 1; i < mMaxBezierLines - 1; i++) {
            ...
            mBezier1[i].x = calculateBezier(mStart.x, mEnd.x, mControl.x, i / (float)
                    mMaxBezierLines);
            mBezier1[i].y = calculateBezier(mStart.y, mEnd.y, mControl.y, i / (float)
                    mMaxBezierLines);
        ...
        for (int i = 1; i < mMaxBezierLines - 1; i++) {
            ...
            mBezier2[i].x = calculateBezier(mStart.x, mEnd.x, mControl.x, i / (float)
                    mMaxBezierLines);
            mBezier2[i].y = calculateBezier(mStart.y, mEnd.y, mControl.y, i / (float)
                    mMaxBezierLines);
        }
    }
